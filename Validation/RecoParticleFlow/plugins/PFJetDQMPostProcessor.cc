...
void PFJetDQMPostProcessor::dqmEndJob(DQMStore::IBooker& ibook_, DQMStore::IGetter& iget_) {
  ...
  for (unsigned int idir = 0; idir < jetResponseDir.size(); idir++) {
    ...
    MonitorElement* me;
    TH1F* h_resp;
    TH1F* h_genjet_pt, *h_genjet_matched_pt;// *h_genjet_unmatched_pt;
    TH1F* h_recojet_pt, *h_recojet_matched_pt, *h_recojet_unmatched_pt;
    ...
    if (debug) {
      ...
      for (std::vector<MonitorElement*>::const_iterator i = vME_presponse.begin(); i != vME_presponse.end(); ++i)
        (*i)->getTH1F()->Print();
      for (std::vector<MonitorElement*>::const_iterator i = vME_presponse_mean.begin(); i != vME_presponse_mean.end(); ++i)
        (*i)->getTH1F()->Print();
      for (std::vector<MonitorElement*>::const_iterator i = vME_presponse_median.begin(); i != vME_presponse_median.end(); ++i)
        (*i)->getTH1F()->Print();
